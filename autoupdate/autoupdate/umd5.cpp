...
bool MD5_file(const char *path, int md5_len, std::string &ret_md5) {
    ...
    MD5Init(&mdContext);
    while ((bytes = (int)fread(data, 1, 1024, fp)) != 0) {
        MD5Update(&mdContext, data, bytes);
    }
    MD5Final(&mdContext);

    ret_md5.resize(md5_len);

    if (md5_len == 16) {
        for (i = 4; i < 12; i++) {
            sprintf(&ret_md5[(i - 4) * 2], "%02x", mdContext.digest[i]);
        }
    } else if (md5_len == 32) {
        for (i = 0; i < 16; i++) {
            sprintf(&ret_md5[i * 2], "%02x", mdContext.digest[i]);
        }
    } else {
        fclose(fp);
        return NULL;
